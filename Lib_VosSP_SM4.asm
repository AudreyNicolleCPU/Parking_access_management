...
_Decod_BIN_to_BCD:	
					...
					;on ajoute à la valeur de DPTR la valeur des bits de poids faibles
					ADD A,DPL
					mov DPL,A
					mov A,DPH
					ADDC A,#0
					mov DPH,A
					;on recupère l'information contenu dans display
					mov A,#0
					movc A,@A+DPTR
					mov R7,A
					Pop ACC
...
_Test_Code:
			...
			mov DPH,R6
			mov DPL,R7
			;on recupère l'info sur le nombre de code contenu par Tab_code
			mov A,#0
			movc A,@A+DPTR
			...
			Verif_code_in:
				INC DPTR
				INC R1
				;on vérifie qu'on est tjs dans Tab_code
				mov A,R0
				ADD A,#1h
				SUBB A,R1
				JZ Code_non_trouve ; si on est sortie, alors notre code 
				;n'existe pas dans nos données
				;recupère données et sauvegarde dans R7
				mov A,#0
				movc A,@A+DPTR
				mov R7,A
				;on vérifie si c'est le bon sinon on continue de chercher
				SUBB A,R5 
				JNZ Verif_code_in 
				Jmp By
				Code_non_trouve:
...
_Stockage_Code:
				...
				;incrémentation index et envoie dans XDATA
				INC R0 
				mov A,R0
				movx @DPTR,A
				
				;on ajoute à DPTR la valeur de l'index pour ranger 
				;le code après la derniere entrée
				mov A,R0
				ADD A,DPL
				mov DPL,A
				mov A,#0
				...
				;envoie des données dans X_DATA
				mov A,R5
				movx @DPTR,A
